...
*** time after whcih the solver terminates:
 ...
 Loop_Probability_results(Loop_Probability,'1')  = 1 - (Loop_Probability.val-1) * 0.1  ;
 Loop_Probability_results(Loop_Probability,'2')  =     (Loop_Probability.val-1) * 0.1  ;
...
***--------------------------------------------------------------------------***
 ...
 Loop(Loop_Probability,
   ...
   probability('1') = 1 - (Loop_Probability.val-1) * 0.1 ;
   probability('2') =     (Loop_Probability.val-1) * 0.1 ;

     Loop(LineInvest,

       lineB(L) = 1$(LineInvest.val=L.val);

*exclude Wind generation in conventional case
*change buVarInv (Input_2Nodes.gms) for each case 68 / 79
$ifthen '%resIntegration%' == off
 genVarInv('1') = 35 ;
 genVarInv('2') = 43 ;
 genVarInv('3') = 68 ;
 genVarInv('4') = 38 ;
...
***--------------------------------------------------------------------------***
  ...
  SP_FLOW(S,L,T)  = f_sp.l(S,L,T)          ;
  SP_CAP_L(L)     = lineB(L) * lineUB(L)   ;

  wf_sc_SP(S)$probability(S)         = ( sum((D,T), ( consObjA(D,T) * SP_DEM(S,D,T) - 0.5 * consObjB(D,T) * SP_DEM(S,D,T) * SP_DEM(S,D,T) ) * periodScale(T) )
                                       - sum((G,T), genVarInv(G) * SP_GEN_G(S,G,T) * periodScale(T) ) ) * YEAR
                                       - sum(G, genFixInv(G) * SP_CAP_G(G)) ;
  wf_SP                              = sum(S$probability(S), probability(S) * wf_sc_SP(S) ) ;
  wf_SP_TEST                         = welfareSpot.l ;
  priceD_Spot(S,D,T) $probability(S) = ( consObjA(D,T) - consObjB(D,T) * SP_DEM(S,D,T) ) ;
  priceG_Spot(S,G,T) $probability(S) = sum(Z$(Z.val = GenInZone(G,S)), ZFKL.m(S,Z,T) / Year / periodScale(T) / probability(S));
...
*  RD_DEM_L(S,D,T) = ls_rd.l(S,D,T) ;
  ...
  ct_RD_TEST      = costRedispatch.l      ;
  Test = sum(S$probability(S), probability(S)
                                         * ( sum((G,T), genVarInv(G) * ( g_rd.l(S,G,T) - SP_GEN_G(S,G,T) ) * periodScale(T) ) * YEAR
                                           + sum((B,T), buVarInv * gb_rd.l(S,B,T) * periodScale(T) ) * YEAR ) )
