...
*** Zonal First Kirchhoffs Law
  Equation ZFKL;
  ZFKL(Z,T,S_co2,S_dloc,S_dlev,S_lcost)..
...
*** Market Coupling Flow Restrictions
  ...
  Equation MCF1;
  MCF1(L,T,S_co2,S_dloc,S_dlev,S_lcost)$(lineInter(L) = 1 and lineIsNew(L) = 0)..   f_sp(L,T,S_co2,S_dloc,S_dlev,S_lcost) =l= lineUB(L);
  Equation MCF2;
  MCF2(L,T,S_co2,S_dloc,S_dlev,S_lcost)$(lineInter(L) = 1 and lineIsNew(L) = 0).. - lineUB(L)=l= f_sp(L,T,S_co2,S_dloc,S_dlev,S_lcost);
  Equation MCF3;
  MCF3(L,T,S_co2,S_dloc,S_dlev,S_lcost)$(lineInter(L) = 1 and lineIsNew(L) = 1)..   f_sp(L,T,S_co2,S_dloc,S_dlev,S_lcost) =l= lineB(L) * lineUB(L);
  Equation MCF4;
  MCF4(L,T,S_co2,S_dloc,S_dlev,S_lcost)$(lineInter(L) = 1 and lineIsNew(L) = 1).. - lineB(L) * lineUB(L) =l= f_sp(L,T,S_co2,S_dloc,S_dlev,S_lcost);

***Generation Capacity Limits
  Equation GCLSpot ;
  GCLSpot(G,T,S_co2,S_dloc,S_dlev,S_lcost)$prob_co2(S_co2)..  g_sp(G,T,S_co2,S_dloc,S_dlev,S_lcost) =l= avail(T,G) * ig_sp(G) ;
...
*** CVAR Restrictions
  ...
  Equation CVARRed;
  CVARRed(S_co2,S_dloc,S_dlev,S_lcost)..
            VAR_rd-
             ((sum((D,T),(consObjA(D,T,S_dloc,S_dlev) * SP_dem(D,T,S_co2,S_dloc,S_dlev,S_lcost)
            - 0.5 * consObjB(D,T,S_dloc,S_dlev) * SP_dem(D,T,S_co2,S_dloc,S_dlev,S_lcost) * SP_dem(D,T,S_co2,S_dloc,S_dlev,S_lcost)) * periodScale(T))
            - sum((G,T), genVarInv(G,S_co2) * g_rd(G,T,S_co2,S_dloc,S_dlev,S_lcost) * periodScale(T))
...
***First Kirchhoffs Law
  ...
  Equation FKL;
  FKL(N,T,S_co2,S_dloc,S_dlev,S_lcost)$prob_co2(S_co2)..   sum(D$(consAtNode(D) = N.val), SP_DEM(D,T,S_co2,S_dloc,S_dlev,S_lcost)) =e=
...
***Second Kirchhoffs Law
  ...
  Equation SKL1;
  SKL1(L,T,S_co2,S_dloc,S_dlev,S_lcost)$(lineIsNew(L) = 0).. f_rd(L,T,S_co2,S_dloc,S_dlev,S_lcost) + lineGamma(L) * (sum(N$(lineStart(L) = N.val), angle(N,T,S_co2,S_dloc,S_dlev,S_lcost)) - sum(N$(lineEnd(L) = N.val), angle(N,T,S_co2,S_dloc,S_dlev,S_lcost))) =e= 0;
  Equation SKL2;
  SKL2(L,T,S_co2,S_dloc,S_dlev,S_lcost)$(lineIsNew(L) = 1).. - M * (1 - lineB(L)) =l=  f_rd(L,T,S_co2,S_dloc,S_dlev,S_lcost) + lineGamma(L) * (sum(N$(lineStart(L) = N.val), angle(N,T,S_co2,S_dloc,S_dlev,S_lcost)) - sum(N$(lineEnd(L) = N.val), angle(N,T,S_co2,S_dloc,S_dlev,S_lcost)));
  Equation SKL3;
  SKL3(L,T,S_co2,S_dloc,S_dlev,S_lcost,S_lcost)$(lineIsNew(L) = 1).. f_rd(L,T,S_co2,S_dloc,S_dlev,S_lcost) + lineGamma(L) * (sum(N$(N.val = lineStart(L)), angle(N,T,S_co2,S_dloc,S_dlev,S_lcost)) - sum(N$(N.val = lineEnd(L)), angle(N,T,S_co2,S_dloc,S_dlev,S_lcost))) =l= M * (1 - lineB(L));

***Voltage Phase Angle

  Equation VPA;
  VPA(N,T,S_co2,S_dloc,S_dlev,S_lcost)$(N.val = 1).. angle(N,T,S_co2,S_dloc,S_dlev,S_lcost) =e= 0;

***Trasmission Flow Limits

  Equation TFL1;
  TFL1(L,T,S_co2,S_dloc,S_dlev,S_lcost)$(lineIsNew(L) = 0)..   f_rd(L,T,S_co2,S_dloc,S_dlev,S_lcost) =l= lineUB(L);
  Equation TFL2;
  TFL2(L,T,S_co2,S_dloc,S_dlev,S_lcost)$(lineIsNew(L) = 0).. - lineUB(L) =l= f_rd(L,T,S_co2,S_dloc,S_dlev,S_lcost);
  Equation TFL3;
  TFL3(L,T,S_co2,S_dloc,S_dlev,S_lcost)$(lineIsNew(L) = 1)..   f_rd(L,T,S_co2,S_dloc,S_dlev,S_lcost) =l= lineB(L) * lineUB(L);
  Equation TFL4;
  TFL4(L,T,S_co2,S_dloc,S_dlev,S_lcost)$(lineIsNew(L) = 1).. - lineB(L) * lineUB(L) =l= f_rd(L,T,S_co2,S_dloc,S_dlev,S_lcost);

***Generation Capacity Limits (Redispatch Level)

  Equation GCLRed;
  GCLRed(G,T,S_co2,S_dloc,S_dlev,S_lcost)$prob_co2(S_co2).. g_rd(G,T,S_co2,S_dloc,S_dlev,S_lcost) =l= avail(T,G) * SP_CAP_G(G) ;

  Equation GCLBu;
  GCLBu(B,T,S_co2,S_dloc,S_dlev,S_lcost)$prob_co2(S_co2)..  gb_rd(B,T,S_co2,S_dloc,S_dlev,S_lcost) =l= ib_rd(B) ;

*** Fix Spot Market and Redispatch Quantities

  Equation fixDem;
  fixDem(D,T,S_co2,S_dloc,S_dlev,S_lcost)$prob_co2(S_co2).. d_rd(D,T,S_co2,S_dloc,S_dlev,S_lcost) =e= SP_DEM(D,T,S_co2,S_dloc,S_dlev,S_lcost)
  ;

  Equation fixGen;
  fixGen(G,T,S_co2,S_dloc,S_dlev,S_lcost)$prob_co2(S_co2).. g_rd(G,T,S_co2,S_dloc,S_dlev,S_lcost) =e= SP_GEN_G(G,T,S_co2,S_dloc,S_dlev,S_lcost) + g_p_rd(G,T,S_co2,S_dloc,S_dlev,S_lcost) - g_n_rd(G,T,S_co2,S_dloc,S_dlev,S_lcost);
